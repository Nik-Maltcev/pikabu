...
async def on_startup():
    """Create database tables and clear stale topic cache."""
    ...
    # Clear old topic cache so fallback list gets re-loaded with correct URLs
    async with async_session() as session:
        await session.execute(delete(Topic))
        await session.commit()
    logger.info("Topic cache cleared for fresh reload.")
